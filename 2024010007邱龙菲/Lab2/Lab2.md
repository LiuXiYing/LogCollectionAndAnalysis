...
> 记录：journald 的实际运行状态为 ____active__。
...
> 记录：Ubuntu 的日期和时间为 ___2026-09-11 16:30:07___；时区为 ____Asia/Shanghai__；时间同步状态为 ____systemd-timesyncd.service active__。
...
> 记录：`/var/log/auth.log` 是否存在？___存在___；实际输出：_____auth.log exists_。
...
> 记录：本次 SSH 连接使用的 Ubuntu 虚拟机 IP 为 ____192.168.43.128__。
...
> 记录：22 端口是否正在监听？____是__；输出中的本地地址和端口为 ___0.0.0.0:22___。
...
> 记录：SSH 登录后的主机名为 ___qlf-VMware-Virtual-Platform___。
...
> 记录：当前目录为 ____/home/qlf__；是否确认已通过 Git Bash 登录 Ubuntu？____是__。
...
> 记录：`/var/log/journal/` 的检查结果为 ___存在目录，属主 systemd-journal，存放持久化 systemd 日志___；`/run/log/journal/` 的检查结果为 ____存在目录，属主 systemd-journal，临时运行时日志存储目录__。
...
| :--- | :--- | :--- | :--- |
| /var/log/syslog| 系统通用日志，记录大部分系统服务运行事件| 文本|cat /var/log/syslog 或 tail /var/log/syslog |
|/var/log/auth.log|认证、登录、sudo 权限操作安全日志|文本 |cat /var/log/auth.log|
|/var/log/wtmp |记录用户登录、登出历史记录 | 二进制| last|
| /var/log/journal|systemd 持久化二进制日志存放目录 | 目录| journalctl|
| /var/log/btmp| 记录登录失败尝试记录| 二进制|lastb |
...
> 记录：选取的一条日志来自哪个程序或服务？___systemd___；记录了什么事件？____sysstat-collect.service 系统活动统计服务执行完成并成功停用__。
...
> 记录：实际查询起点为 ____2026-09-09 08:00:00__；终点为 ___2026-09-09 12:00:00___；观察到的事件或无记录情况为 ____No entries，该时间段内没有日志记录__。
...
> 记录：是否查到匹配日志？____是__；其中一条的内容或无记录提示为 ___kernel: Speculative Return Stack Overflow: WARNING: See [https://kernel.org/](https://kernel.org/)... 内核存在推测返回栈溢出相关警告___。
...
> 填写：/var/log/wtmp` 和 `/var/log/btmp` 是二进制格式文件，不是文本文件，`cat`读取会输出乱码。查看`wtmp`使用`last`命令，查看`btmp`使用`lastb`命令。
