...
>
> 答：在默认安装的 Ubuntu 系统中，该目录下除`50‑default.conf`之外，没有其他.conf 文件。
...
> 填写：日志设施是user，不是 auth/authpriv，匹配规则 `*.*;auth,authpriv.none  -/var/log/syslog`；`auth,authpriv.*`这条规则只接收 auth、authpriv 设施日志，user 设施不匹配 auth.log 规则，所以写入 syslog，不会进入 auth.log

> **自检题 ②**：sshd 的登录日志为什么**只**出现在 `auth.log`，不出现在 `syslog`？提示：看 `*.*;auth,authpriv.none` 这一行里的 `none`。

> 填写：sshd 登录日志属于`auth、authpriv`设施，匹配`auth,authpriv.*  /var/log/auth.log`写入 auth.log；而 syslog 的规则 `*.*;auth,authpriv.none` 中，`auth,authpriv.none`代表过滤、排除 auth 与 authpriv 设施的日志，所以不会再写入 syslog，避免认证日志重复保存
...
| :--- | :---: | :---: | :--- |
| case=1 `user.info` | 是|否 |	该日志设施为 user，匹配规则*.*;auth,authpriv.none，写入 syslog；不匹配auth,authpriv.*，所以不会进入 auth.log。|
| case=2 `auth.info` | 否|是 |命中`auth,authpriv.*`规则写入 auth.log；syslog 规则中`auth,authpriv.none`将 auth 设施排除，因此不会写入 syslog |
| case=3 `cron.info` | 是|否 |配置文件中 cron 相关输出规则被注释，无独立日志文件；消息被`*.*`通用规则捕获写入 syslog，不属于认证类设施，不进入 auth.log。 |
| case=4 `local6.err` | 是|否 |local6 为用户自定义本地设施，没有配置专属输出规则；被`*.*`通用规则捕获写入 syslog，不属于 auth 类设施，不进入 auth.log。 |
...
>
> 答：能查到，可以看到输出 `facility=local6 priority=info msg=hello`
>
> ② `syslog` 里能查到吗？参照上面的流向图，说明同一条消息为什么三处都有记录。
>
> 答：syslog 中也能查到。rsyslog 的规则是匹配后不会丢弃消息，消息会继续向下传递；自定义规则把 local6 写入 local6.log，系统默认规则又把全部信息写入 /var/log/syslog；同时 systemd‑journal 会捕获全部系统日志，所以同一条日志会同时出现在三处。
>
> ③ `journalctl` 里能查到吗？
>
> 答：能查到，journalctl‑t loglab 可以抓取到该条日志。
>
> ④ 新文件的属主、属组和权限分别是什么？
>
> 答：权限：‑rw‑r‑‑‑‑‑（640）；属主：syslog；属组：adm
...
>
> 答：有认出，输出可见 `rotating pattern: /var/log/loglab/local6.log  after 1 days (7 rotations)`，识别到该日志文件。
>
...
1. facility 和 priority 分别表示什么？`mail.err` 匹配的是"只有 err"还是"err 及以上"？
   ...
   > 填写：facility 是设施，代表日志来源模块（如 mail、auth）；priority 是优先级，代表日志严重等级。`mail.err`匹配**err 及以上**等级的日志

2. 为什么日志需要轮转，而不是一直往同一个文件里写？请至少说出两个理由。

   > 填写：防止单个日志文件体积无限膨胀，占满磁盘空间。大日志文件查询、归档、压缩都很慢，轮转拆分后便于管理、备份与旧日志清理

3. `logrotate -d` 和 `logrotate -f` 的区别是什么？改完 logrotate 配置后，为什么标准动作是先 `-d`，再等定时器？

   > 填写：`‑d`是调试模式，只模拟输出流程，不会实际修改文件；`‑f`是强制轮转，无视轮转条件直接执行轮转。先执行`‑d`可以检查配置语法错误，避免错误配置被定时任务执行破坏系统日志，确认无误后等待定时任务自动执行。
