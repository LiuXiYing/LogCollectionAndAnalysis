...
| 安装介质的版本 | Noble Numbat" -Release amd64 |
| 处理器架构 |x86_64  |
...
| 桌面分辨率是否能自动调整 | 是，可自适应调整|
| SSH 是否 enabled / active | enabled / active|
| 22 端口是否监听 |已监听（0.0.0.0:22、[::]:22）|
| rsyslog 是否 enabled / active | enabled / active|
| `/var/log/syslog` 是否出现带学号姓名的测试日志 |是，成功生成 lab1‑check: Lab1 rsyslog test qlf |
| 三项组件是否全部验收合格 |全部合格 |
...
| 验收项目 | 合格标准 | 你的结论 |
| :--- | :--- | :--- |
| VMware 版本 | VMware Workstation Pro 26H1 for Windows | 安装成功|
| Linux 版本 | Ubuntu 24.04 LTS Desktop amd64，安装介质为教师提供的 24.04.4 |   正在运行|
| 虚拟机联网 | 具有 IP 和默认路由，IP 联通与 DNS 解析正常 |联网正常，获取 IP，外网连通、DNS 解析正常 |
| 国内软件源 | 已换成国内镜像站，`sudo apt update` 成功 |已更换国内镜像源，`sudo apt update` 更新成功 |
| CPU、内存、存储 | 至少 2 核、4GB、40GB，且与宿主机档位匹配 | 配置达标，≥2 核、4GB 内存、40GB 硬盘|
