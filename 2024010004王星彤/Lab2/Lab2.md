...
> 记录：Ubuntu 的日期和时间为 2026-09-11 13:48:00；时区为Asia/Shanghai ；时间同步状态为 yes。
...
> 记录：本次 SSH 连接使用的 Ubuntu 虚拟机 IP 为 172.20.10.2。
...
> 记录：SSH 登录后的主机名为 wxt-VMware-Virtual-Platform
。
...
| :--- | :--- | :--- | :--- |
|/var/log/syslog |系统全局运行日志 |文本 |cat / less|
|/var/log/auth.log |用户登录、SSH认证日志|文本 |cat / less |
|/var/log/journal	systemd |持久化journal日志目录 |二进制|journalctl	 |
|/run/log/journal |临时journal日志目录 |二进制 |journalctl |
...
> 记录：实际查询起点为2026-09-11 14:10:00 ；终点为 2026-09-11 14:40:00；观察到的事件或无记录情况为 成功输出该时间段日志，包含sudo执行、用户会话打开/关闭、系统服务启停、定时任务CRON、网络相关事件。
...
> 记录：是否查到匹配日志？是；其中一条的内容或无记录提示为 gnome-shell[1737]: JS ERROR: Gio.IOErrorEnum: Xwayland exited unexpectedly。
...
> 填写：/var/log/wtmp 和 /var/log/btmp 是二进制日志文件，不是纯文本，cat读取只会显示乱码。读取wtmp用last命令，读取btmp用lastb命令。
