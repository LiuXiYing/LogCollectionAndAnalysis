...
| 项目 | 你的填写内容 |
| :--- | :--- |
| 宿主机内存 / CPU 核心 / 存放盘剩余空间 |CPU核心2核，总内存5.7Gi，根分区剩余可用25G	 |
| 选择的配置档位 | 课程推荐档  |
| 虚拟 CPU 核心数 |2 |
| 虚拟内存 |5.7Gi（6Gi） |
| 虚磁盘容量 |40G |
...
| 项目 | 你的填写内容 |
| :--- | :--- |
| VMware Tools 版本 |open‑vm‑tools（13.3.0）	 |
...
| 验收项目 | 合格标准 | 你的结论 |
| :--- | :--- | :--- |
| VMware 版本 | VMware Workstation Pro 26H1 for Windows |版本符合要求，验收合格	 |
| Linux 版本 | Ubuntu 24.04 LTS Desktop amd64，安装介质为教师提供的 24.04.4 |系统版本匹配，验收合格	 |
| 虚拟机联网 | 具有 IP 和默认路由，IP 联通与 DNS 解析正常 |获取IP地址，外网连通正常，验收合格	 |
| 国内软件源 | 已换成国内镜像站，`sudo apt update` 成功 |国内源配置生效，更新无报错，验收合格	 |
| CPU、内存、存储 | 至少 2 核、4GB、40GB，且与宿主机档位匹配 |分配2核CPU、约6Gi内存、40G磁盘，资源达标，验收合格	 |
| VMware Tools、SSH、rsyslog | 软件包已安装，服务和功能检查通过 |三项服务均开机自启且运行正常，日志测试成功，验收合格	 |
