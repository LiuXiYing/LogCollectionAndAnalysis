...
> 记录：本次使用的 Ubuntu 用户名为 __ubuntu____。
...
> 记录：本次 SSH 连接的 Ubuntu 目标 IP 为 ____192.168.118.128__。
...
> 记录：本机主机名为 __ubuntu-VMware-Virtual-Platform____（即 4W1R 中 Where 的取值）。
...
| :--- | :--- | :--- | :--- | :--- | :--- |
| 成功认证 |2026-09-25T17:39:03.273070+08:00 |ubuntu |192.168.118.128 |Accepted password |/var/log/auth.log |
| 失败认证 |2026-09-25T17:38:56.282351+08:00 |ubuntu |192.168.118.128 |Failed password |/var/log/auth.log |
...
| 项目 | 你的记录 |
| :--- | :--- |
| journal 中是否查到 |是，成功查询到日志记录 |
| `/var/log/syslog` 中是否查到 |是，成功查询到日志记录 |
| 两处记录有哪些共同字段或正文 |标签`lab3_read`、日志正文完全一致：`student_id=2024010018 name=tianzhenwei action=write_test result=success`；事件发生时间、主机名相同 |
| 两处输出的主要区别 |journalctl 输出：时间格式简化（`9月 25 18:03:28`），附带进程 PID`[6851]`；/var/log/syslog 输出：使用 ISO 标准时间格式`2026-09-25T18:03:28.636092+08:00`，无进程 PID 字段 |
...
```text
获取命令：sudo grep -F "student_id=2024010018" /var/log/syslog | tail -n 5
日志原文：2026-09-25T18:03:28.636092+08:00 ubuntu-VMware-Virtual-Platform lab3_read: student_id=2024010018 name=tianzhenwei action=write_test result=success
```
...
> 填写：2026 年 9 月 25 日 18:03:28，我在 ubuntu 虚拟机主机上，使用 logger 工具写入一条带学号姓名的自定义测试日志，日志同时被 journald 与 rsyslog 捕获保存，消息标记测试结果为 success。
...
日志原文：2026-09-25T17:39:03.273070+08:00 ubuntu-VMware-Virtual-Platform sshd[3810]: Accepted password for ubuntu from 192.168.118.128 port 45416 ssh2
```

| 4W1R | 根据本人原始日志填写 |
| :--- | :--- |
| When 什么时候 |2026 年 09 月 25 日 17:39:03 |
| Where 在哪里 |本机主机`ubuntu-VMware-Virtual-Platform`；登录来源 IP：`192.168.118.128`，端口`45416` |
| Who 谁 |记录程序是`sshd`；尝试登录账号为`ubuntu` |
| What 做了什么 |客户端通过 SSH 协议尝试使用密码登录本机 ubuntu 账户 |
| Result 结果如何 |`Accepted password`，密码认证成功，SSH 登录通过 |

**用一两句话解释这个事件：**

> 填写：2026 年 9 月 25 日 17:39:03，从 IP 地址 192.168.118.128 的客户端，使用 SSH 连接虚拟机，对 ubuntu 账号进行密码认证，认证成功。
...
```text
实际日志来源（使用替代来源时说明原因）：Ubuntu虚拟机 /var/log/dpkg.log
获取命令：grep "htop" /var/log/dpkg.log | tail -n 10
日志原文：2026-09-25 18:10:00 status installed htop:amd64 3.2.1-3build1
```

| 4W1R | 根据本人原始日志填写 |
| :--- | :--- |
| When 什么时候 |2026 年 09 月 25 日 18:10:00；日志未提供时区 |
| Where 在哪里 |从Ubuntu虚拟机的 `/var/log/dpkg.log` 获取 |
...
> 填写：9 月 25 日 18:10，软件包管理工具 dpkg 记录 htop 软件包为已安装状态，本条日志没有记录执行该操作的用户。
...
1. systemd-journald 与 rsyslog 各负责什么？结合 3.2 节的结果，解释它们为什么可以同时保留同一条日志。
   ...
   > 填写：systemd-journald 是 systemd 自带日志服务，收集系统、服务输出日志，存为二进制 journal；rsyslog 是传统 syslog 服务，接收日志并保存为文本日志文件。journald 收到日志后，会转发一份给 rsyslog，因此二者可以各自存储同一条日志。

2. SSH 提示 `Failed password` 能证明什么，不能证明什么？请结合本次记录回答。

   > 填写：`Failed password`可以证明：有来自对应 IP 的客户端尝试 SSH 登录目标账号，密码校验失败。它不能证明这个 IP 对应的就是真实攻击者，也无法确认操作人身份，仅能看到登录尝试行为。
