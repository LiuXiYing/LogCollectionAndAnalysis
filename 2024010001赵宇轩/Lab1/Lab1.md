...
| 项目 | 你的填写内容 |
| :--- | :--- |
| 虚拟机 IP 地址 | 192.168.153.128|
| 网络模式 | NAT  |
| ping `223.5.5.5` 是否成功 | 成功，0%丢包|
| ping `mirrors.tuna.tsinghua.edu.cn` 是否成功 |成功，0%丢包 |
| 使用的软件源镜像站 |清华大学镜像源 mirrors.tuna.tsinghua.edu.cn |
...
| 项目 | 你的填写内容 |
| :--- | :--- |
| VMware Tools 版本 |2:13.0.10‑0ubuntu0.24.04.1 |
| `open-vm-tools` 是否 active |是 |
| 桌面分辨率是否能自动调整 |是，拖动窗口可自动适配分辨率 |
| SSH 是否 enabled / active |enabled；active（开机自启且正在运行） |
| 22 端口是否监听 |是，0.0.0.0:22、[::]:22 处于LISTEN监听状态 |
| rsyslog 是否 enabled / active |enabled；active（开机自启且正在运行） |
| `/var/log/syslog` 是否出现带学号姓名的测试日志 | 是，存在 lab1‑check: Lab1 rsyslog test 2024010001赵宇轩 测试记录|
| 三项组件是否全部验收合格 | 全部合格（open‑vm‑tools、ssh‑server、rsyslog均正常）|
...
| 虚拟机联网 | 具有 IP 和默认路由，IP 联通与 DNS 解析正常 | 具有IP和默认路由，IP联通与DNS解析正常|
| 国内软件源 | 已换成国内镜像站，`sudo apt update` 成功 |已换成国内镜像站，sudo apt update 成功 |
| CPU、内存、存储 | 至少 2 核、4GB、40GB，且与宿主机档位匹配 |至少2核、4GB、40GB，且与宿主机档位匹配 |
| VMware Tools、SSH、rsyslog | 软件包已安装，服务和功能检查通过 |软件包已安装，服务和功能检查通过 |

简要说明你遇到的问题、解决方法，以及当前环境是否可以继续完成后续实验：

> 填写：配置国内镜像源时，第一次执行sudo apt update出现域名解析超时。排查后确认是虚拟机DNS问题，修改DNS服务器地址后，更新命令执行成功。其余项目配置正常，当前环境可以继续完成后续实验。
