...
| 项目 | 你的填写内容 |
| :--- | :--- |
| 宿主机内存 / CPU 核心 / 存放盘剩余空间 |20224MB (≈20GB) 内存，8 核 CPU，磁盘剩余空间充足 |
...
| 22 端口是否监听 |是，0.0.0.0:22 和 [::]:22 处于 LISTEN 监听状态 |
| rsyslog 是否 enabled / active |enabled、active|
| `/var/log/syslog` 是否出现带学号姓名的测试日志 | 是，日志成功写入：lab1-check: Lab1 rsyslog test 2024010021lsn|
| 三项组件是否全部验收合格 |是 |
...
> 填写：
本次环境验收实验所有命令执行顺利，网络连通、系统资源分配、VMware‑Tools、SSH 服务与 rsyslog 日志写入全部验收通过，无重大故障。当前虚拟机实验环境完全合格，可以开展后续课程实验。