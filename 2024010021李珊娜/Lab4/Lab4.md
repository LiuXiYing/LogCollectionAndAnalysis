...
> 填写：它不匹配 `auth,authpriv.*`，但匹配 `*.*;auth,authpriv.none`（user 未被 none 排除），所以只进 syslog。

> **自检题 ②**：sshd 的登录日志为什么**只**出现在 `auth.log`，不出现在 `syslog`？提示：看 `*.*;auth,authpriv.none` 这一行里的 `none`。

> 填写：auth.log 有规则 `auth,authpriv.*` 接收它；syslog 那行 `*.*;auth,authpriv.none` 用 none 把 auth 排除了。
...
| :--- | :---: | :---: | :--- |
| case=1 `user.info` | 是| 否|匹配 `*.*`，未被 none 排除 |
| case=2 `auth.info` |否 | 是| 匹配 `auth,authpriv.*`；syslog 行用 none 排除了 auth|
| case=3 `cron.info` | 是| 否|匹配 `*.*`，未被 none 排除 |
| case=4 `local6.err` |是 | 否|匹配 `*.*`，目前没有专属文件 |
...
>
> 答：能。journald 先收进二进制日志；rsyslog 的 60-loglab 规则命中写入 local6.log；50-default 的 `*.*` 也命中写入 syslog。一条消息可匹配多条规则。
>
> ③ `journalctl` 里能查到吗？
>
> 答：能
...
> ② 这台虚拟机下次自动轮转的时间是？
>
> 答：2026-09-27（周日）00:00:00 CST，即约 6 小时后
...
>
> 答：有。输出 `reading config file /etc/logrotate.d/loglab-local6` 读取了配置，最后 `Handling 1 logs` 表示识别并处理 1 个日志（即 local6.log）
>
> ② 有没有报 `parent directory has insecure permissions`？
>
> 答：没有。输出中没有该警告，说明权限检查通过，**不需要**加 `su syslog adm`
...
> 填写：写进 `.1`（旧 inode）。rsyslog 的 fd 仍指向旧文件（inode 678855），logrotate 只是把名字改成 .1，进程不知道改名，继续往旧 inode 写。
...
> ② 和轮转前相比，变化了吗？
>
> 答：变了（678855 → 659980）。名字没变、实体换了：新 local6.log 是空文件，旧内容被改名压缩成 .1.gz。
...
> ② 括号里的状态词是什么？它意味着什么？
>
> 答：`(deleted)`。意味着旧 inode（678855）已被删除，但 rsyslog 进程仍持有指向它的文件描述符，继续往里写 —— 所以 `after-rotate test-1` 被写进已删除的文件，`cat local6.log` 为空，日志丢失。
...
>
> 答：- `local6.log`（含 test-3，postrotate 生效后直接写入新文件）
- `local6.log.1`（未压缩，delaycompress 生效）
- `local6.log.2.gz`（已压缩）
- `local6.log.3.gz`（已压缩）
...
1. facility 和 priority 分别表示什么？`mail.err` 匹配的是"只有 err"还是"err 及以上"？
   ...
   > 填写：facility（设施）表示日志的**类别**，回答 "这条日志是哪一类"，如 user（普通用户程序）、auth（认证与安全）、cron（定时任务）、local6（自定义）等；priority（级别）表示日志的**严重程度**，回答 "有多严重"，共 8 级，数字越小越严重（emerg 0 ~ debug 7）。`mail.err` 匹配的是 **err 及以上**（err /crit/alert /emerg）；只有写成 `mail.=err`（带等号）才是精确匹配 err 这一级。

2. 为什么日志需要轮转，而不是一直往同一个文件里写？请至少说出两个理由。

   > 填写：① 日志会持续增长，一直写下去会占满磁盘空间，导致系统异常甚至崩溃；② 单个日志文件过大后，查询、阅读、备份和传输都很困难，影响排障效率；③ 轮转可以对旧文件压缩、按保留数量清理，在控制磁盘占用的同时保留历史记录。

3. `logrotate -d` 和 `logrotate -f` 的区别是什么？改完 logrotate 配置后，为什么标准动作是先 `-d`，再等定时器？

   > 填写：-d（debug）是演练模式，只打印 "打算做什么"，不真正轮转，用于检查配置是否正确识别；-f（force）是强制模式，无视上次轮转时间记录，立刻真实执行一次轮转。改完配置先 -d，是为了在不动真格的前提下确认配置无误，避免配置写错导致真实轮转时出问题；确认无误后，日常就交给定时器（logrotate.timer，每天一次）自动执行，不需要手动干预。
