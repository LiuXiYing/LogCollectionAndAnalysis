...
> 记录：Ubuntu 的日期和时间为 ___2026-09-11 23:35:53 CST___；时区为 ___Asia/Shanghai (CST, +0800)___；时间同步状态为 ___yes___。
...
> 记录：`/var/log/syslog` 是否存在？___是___；实际输出：___syslog exists_。
...
> 记录：本次 SSH 连接使用的 Ubuntu 虚拟机 IP 为 ___192.168.156.128___。
...
> 记录：SSH 登录后的主机名为 ___xdj-VMware-Virtual-Platform___。
...
> 记录：当前目录为 ___/home/xdj___；是否确认已通过 Git Bash 登录 Ubuntu？___是___。
...
> 记录：`/var/log/journal/` 的检查结果为 ____drwxr-sr-x，root systemd-journal__；`/run/log/journal/` 的检查结果为 ___drwxr-sr-x，root systemd-journal___。
...
| :--- | :--- | :--- | :--- |
|/var/log/syslog |系统综合日志，记录大部分系统服务运行信息 | 文本| tail / cat / less|
|/var/log/auth.log |身份认证日志，SSH 登录、账号权限相关记录 |文本 | tail / cat / less|
| /var/log/kern.log|内核运行日志，硬件、内核报错信息 |文本 |tail / cat / less |
|/var/log/dmesg |内核启动时硬件检测日志 |文本 |dmesg / cat |
...
> 记录：选取的一条日志来自哪个程序或服务？___来自 sudo 程序（pam_unix 认证模块___；记录了什么事件？___用户 xdj 执行 sudo 命令，成功打开 root 会话___。
...
> 记录：实际查询起点为 ___2026-09-12 00:00:00___；终点为 ___2026-09-12 12:00:00___；观察到的事件或无记录情况为 ___查询到多条系统服务启停日志，包含 apt-upgrade、sysstat-collect、logrotate、cups 服务等系统定时任务与服务启动关闭事件___。
...
> 记录：是否查到匹配日志？___是___；其中一条的内容或无记录提示为 ___Dependency failed for sssd-nss.socket - SSSD NSS Service responder socket___。
...
> 填写：/var/log/wtmp 和 /var/log/btmp 是二进制日志文件，不是纯文本，cat 读取只会输出乱码。读取wtmp使用last命令，读取btmp使用lastb命令。
