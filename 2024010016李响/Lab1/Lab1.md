...
| 项目 | 你的填写内容 |
| :--- | :--- |
| 已安装的 VMware 完整版本号 | VMware Workstation Pro 26H1，Version 26.0.0 build-25388281 |
| 是否为教师指定版本 | 是 |
...
| 项目 | 你的填写内容 |
| :--- | :--- |
| 虚拟机 IP 地址 | 192.168.18.128 |
| 网络模式 | NAT |
| ping `223.5.5.5` 是否成功 | 是 |
| ping `mirrors.tuna.tsinghua.edu.cn` 是否成功 | 是 |
| 使用的软件源镜像站 | 清华大学开源软件镜像站 mirrors.tuna.tsinghua.edu.cn |
...
| 项目 | 你的填写内容 |
| :--- | :--- |
| 宿主机内存 / CPU核心 / 存放盘剩余空间 | 15540 MB / 8 核 16 线程 / C 盘约 125 GB，D 盘约 160 GB |
| 选择的配置档位 | 课程推荐档 |
| 虚拟 CPU 核心数 | 2 |
| 虚拟内存 | 5.8 GiB（VMware 配置 6144 MB） |
| 虚磁盘容量 | 60 GB |
| 根分区可用空间 | 45 GB |
| 资源分配是否符合对应档位 | 是 |
...
```bash
logger -t lab1-check "Lab1 rsyslog test 2024010016李响"
```
...
> 1. 建虚拟磁盘的时候 VMware 自带的工具总是报错，弄了半天，最后是手动建了磁盘文件、自己写好配置，才把系统装上。
> 2. 装好系统后想远程截几张图，终端能打开但截出来是空白桌面，查了一下是缺 X 授权，把授权文件加上、换成 xterm 就好了。
> 3. 截图截到一半屏幕自动锁屏，拍出来全是锁屏画面，把自动锁屏关掉重新截就正常了。
>
> 目前环境各项检查都通过了，可以继续做后面的实验。
