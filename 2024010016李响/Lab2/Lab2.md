...
> 记录：Ubuntu 的日期和时间为 **2026-09-10 09:25:14 CST**；时区为 **Asia/Shanghai**；时间同步状态为 **System clock synchronized: yes（NTP active）**。
...
> 记录：本次 SSH 连接使用的 Ubuntu 虚拟机 IP 为 **192.168.18.128**。
...
> 记录：22 端口是否正在监听？**是（有 LISTEN 记录）**；输出中的本地地址和端口为 **0.0.0.0:22 和 [::]:22**。
...
> 记录：当前目录为 **/home/lixiang**；是否确认已通过 Git Bash 登录 Ubuntu？**是**。
...
> 记录：`/var/log/journal/` 的检查结果为 **目录存在（drwxr-sr-x+，属主 root:systemd-journal）**；`/run/log/journal/` 的检查结果为 **目录存在（drwxr-sr-x+）**。
...
| :--- | :--- | :--- | :--- |
| `/var/log/syslog` | 系统与服务的综合 syslog 消息 | 文本（`file` 显示 ASCII text） | `sudo tail`、`sudo grep` |
| `/var/log/auth.log` | SSH、sudo、登录认证与授权事件 | 文本（`file` 显示 ASCII text） | `sudo tail`、`sudo grep` |
| `/var/log/kern.log` | 内核、驱动、硬件和 OOM 等消息 | 文本 | `sudo tail`（也可 `journalctl -k`） |
| `/var/log/dpkg.log` | 软件包安装、升级、删除和状态变化 | 文本 | `sudo tail`、`sudo grep` |
| `/var/log/wtmp` | 登录成功、注销和重启历史 | 二进制（`file` 显示 data） | `last` |
...
> 记录：选取的一条日志来自 **sshd**；记录了 **lixiang 从 192.168.18.1 通过密码认证成功登录 SSH（Accepted password for lixiang from 192.168.18.1 port 49281 ssh2），随后 systemd-logind 创建了新会话**。
...
> 记录：实际查询起点为 **2026-09-10 09:00:00**；终点为 **2026-09-10 09:30:00**；观察到的事件为 **该时段内有 SSH 会话建立与断开（sshd 接受 lixiang 密码登录、session 打开/关闭），以及 lixiang 用 sudo 执行 `ls`、`file`、`tail`、`journalctl` 等命令的审计记录**。
...
> 记录：是否查到匹配日志？**是**；其中一条的内容为 **`sshd[5641]: error: kex_exchange_identification: read: Connection reset by peer`（SSH 连接方在密钥交换阶段被重置）；另有 systemd-resolved 提示 DNS 服务器 192.168.18.2 在 UDP/TCP 间降级切换的 warning**。
...
> 填写：
>
> `/var/log/wtmp` 和 `/var/log/btmp` 是二进制格式的登录数据库，用 `cat` 直接读取只会输出乱码，还可能干扰终端显示。查看成功登录、注销和重启历史应使用 `last`（读取 wtmp），查看失败登录历史应使用 `sudo lastb`（读取 btmp）。
