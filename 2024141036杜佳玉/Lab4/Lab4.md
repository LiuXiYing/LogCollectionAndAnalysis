...
> 填写：日志 facility 是 user，不属于 auth/authpriv，匹配 `*.*;auth,authpriv.none` 规则，写入 `/var/log/syslog`；auth.log 的规则只捕获 auth/authpriv 设施日志，因此不会写入 auth.log。

> **自检题 ②**：sshd 的登录日志为什么**只**出现在 `auth.log`，不出现在 `syslog`？提示：看 `*.*;auth,authpriv.none` 这一行里的 `none`。

> 填写：sshd 登录日志属于 auth/authpriv 设施。`*.*;auth,authpriv.none` 规则表示：所有日志写入 syslog，但 auth、authpriv 设施日志不写入 syslog（none = 丢弃）；auth,authpriv.* 单独匹配，写入 /var/log/auth.log，因此只出现在 auth.log。
...
| 命令 | 先猜：会落到哪个文件？ |
| :--- | :--- |
| case=1 `user.info` |`/var/log/user.log`，同时匹配`*.*;auth,authpriv.none`也会写入`/var/log/syslog`|
| case=2 `auth.info` |`/var/log/auth.log`；`*.*;auth,authpriv.none`会过滤掉 auth，不会进 syslog|
| case=3 `cron.info` |`/var/log/cron.log`，同时也写入`/var/log/syslog`|
| case=4 `local6.err` |`/var/log/syslog`（配置文件没有单独给 local6 定义规则，匹配`*.*;auth,authpriv.none`）|
...
| :--- | :---: | :---: | :--- |
| case=1 `user.info` |是|否|`user.info`匹配`user.*`写入 user.log，同时匹配`*.*;auth,authpriv.none`，不属于 auth/authpriv，写入 syslog；auth.log 只匹配 auth、authpriv 设施，所以不会进入 auth.log|
| case=2 `auth.info` |否|是|`auth.info`匹配`auth,authpriv.*`写入 auth.log；`*.*;auth,authpriv.none`对 auth 设施设置 none，被排除，不会写入 syslog|
| case=3 `cron.info` |是|否|`cron.info`匹配`cron.*`写入 cron.log，同时匹配`*.*;auth,authpriv.none`，不属于 auth/authpriv，写入 syslog；不会匹配 auth 相关规则|
| case=4 `local6.err` |是|否|配置里没有 local6 专属分流规则，匹配`*.*;auth,authpriv.none`，不属于 auth/authpriv，写入 syslog；不匹配 auth 规则，不会进入 auth.log|
...
>
> 答：可以查到，消息发送后，`systemd-journald`先接收存入 journal；rsyslog 收到消息，匹配两条规则：`local6.*`写入`local6.log`，全局`*.*`写入`syslog`。同一条日志消息可以匹配多条 rsyslog 规则，命中的所有规则都会执行，所以在 local6.log 和 syslog 都保存；journald 是独立的日志系统，会捕获所有系统消息，所以 journalctl 也能看到。
>
> ③ `journalctl` 里能查到吗？
>
> 答：可以查到。
...
>
> 答：**有，成功识别该日志文件**（输出显示 `Handling 1 logs`、`considering log /var/log/loglab/local6.log`）。
>
...
> ② 括号里的状态词是什么？它意味着什么？
>
> 答：状态词是`deleted`；文件在目录上被改名归档，但 rsyslog 进程仍然持有这个 inode 的文件句柄，内核不会真正删除这块磁盘空间，进程还可以继续向这个旧 inode 写入数据
...
1. facility 和 priority 分别表示什么？`mail.err` 匹配的是"只有 err"还是"err 及以上"？
   ...
   > 填写：facility（设施）代表日志的来源模块（如 mail、local6）；priority（优先级）代表日志严重等级。`mail.err`匹配**err 及以上**更严重级别的日志。

2. 为什么日志需要轮转，而不是一直往同一个文件里写？请至少说出两个理由。

   > 填写：1.防止单个日志文件无限膨胀，占满磁盘空间；2.方便归档、压缩与历史日志清理，便于日志查看和故障排查。

3. `logrotate -d` 和 `logrotate -f` 的区别是什么？改完 logrotate 配置后，为什么标准动作是先 `-d`，再等定时器？

   > 填写：`logrotate -d`是**调试预览**，只模拟轮转、不实际修改文件；`logrotate -f`强制立刻执行真实轮转。改配置先使用`-d`，用于检查配置语法与执行逻辑，避免错误配置直接破坏日志，确认无误后再等待定时任务自动执行。
