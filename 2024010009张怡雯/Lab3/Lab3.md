...
> 记录：本次使用的 Ubuntu 用户名为 __zyw____。
...
> 记录：本次 SSH 连接的 Ubuntu 目标 IP 为 __192.168.134.129____。
...
> 记录：本机主机名为 __zyw-VMware-Virtual-Platform____（即 4W1R 中 Where 的取值）。
...
| :--- | :--- | :--- | :--- | :--- | :--- |
| 成功认证 |2026-09-23 17:56:07 |zyw |192.168.134.129 |Accepted password |journalctl / auth.log |
| 失败认证 |2026-09-24 08:18:00|wrong |127.0.0.1 |Failed password |journalctl / auth.log |
...
| :--- | :--- |
| journal 中是否查到 |已查到 |
| `/var/log/syslog` 中是否查到 |已查到 |
| 两处记录有哪些共同字段或正文 |student_id=2024010009 name=zyw action=write_test result=success
 |
| 两处输出的主要区别 | 显示格式不同。 1. journalctl 显示的时间是 9月 24 08:24:55，主机名后面带有 lab3_read[7448]: 标签。 2. syslog 显示的时间格式通常是 Sep 24 08:24:55（英文月份），且前面有具体日期和主机名，lab3_read 标签位于主机名之后，中间没有 [进程ID] 这样的方括号数字 |
...
日志原文：9月 24 08:24:55 zyw-VMware-Virtual-Platform lab3_read[7448]: student_id=2024010009 name=zyw action=write_test result=success
```

| 4W1R | 根据本人原始日志填写 |
| :--- | :--- |
| When 什么时候 |9月24日 08:24:55 |
| Where 在哪里 |主机 zyw-VMware-Virtual-Platform |
| Who 谁 |自己指定的标签 lab3_read (进程号 7448) 写入；正文中标识的本人姓名为 zyw，学号 2024010009 |
| What 做了什么 |向本机日志系统写入一条 write_test 测试消息 |
| Result 结果如何 |正文写明 result=success，这是自己写入的测试标记，需结合两处查询均能查到来印证 |

**用一两句话解释这个事件：**

> 填写：9月24日 08:24:55，本人在 zyw-VMware-Virtual-Platform 主机上用 logger 写入一条学号为 2024010009 的测试消息，正文标记结果为 success
...
日志原文：2026-09-23T17:56:07.978979+08:00 zyw-VMware-Virtual-Platform sshd[3581]: Accepted password for zyw from 192.168.134.129 port 35882 ssh2
```

| 4W1R | 根据本人原始日志填写 |
| :--- | :--- |
| When 什么时候 |9月23日 17:56:07 |
| Where 在哪里 |主机 zyw-VMware-Virtual-Platform |
| Who 谁 |记录程序为 sshd（进程号 3581）；尝试登录的账号为 zyw |
| What 做了什么 | 从来源 IP 192.168.134.129 的端口 35882，通过 SSH 协议尝试密码登录 |
| Result 结果如何 |日志明确显示 Accepted password，表示密码认证成功，建立会话 |

**用一两句话解释这个事件：**

> 填写：9月23日 17:56:07，账号 zyw 从 IP 192.168.134.129 发起 SSH 连接，成功通过密码验证登录到主机 zyw-VMware-Virtual-Platform
...
日志原文：2026-09-24 09:28:34 status half-installed htop:amd64 3.3.0-4build1
```

| 4W1R | 根据本人原始日志填写 |
| :--- | :--- |
| When 什么时候 |9月24日 09:35:01 |
| Where 在哪里 |从本人 Ubuntu 虚拟机的 /var/log/dpkg.log 取得 |
| Who 谁 |记录工具为 dpkg；该日志未提供执行操作的用户账号 |
| What 做了什么 |记录 htop 软件包的状态 |
| Result 结果如何 |状态为 status installed，即已安装；仅凭这一行不能判断此前执行的是首次安装还是升级 |

**用一两句话解释这个事件：**

> 填写：2026-09-24 09:28:34，软件包管理工具记录了 htop 已安装的状态。这一行没有说明是哪位用户执行的操作
...
1. systemd-journald 与 rsyslog 各负责什么？结合 3.2 节的结果，解释它们为什么可以同时保留同一条日志。
   ...
   > 填写：systemd-journald 负责收集和管理系统启动以来的所有日志，采用二进制格式存储（可通过 journalctl 查询）；而 rsyslog 则负责将收集到的日志分类写入到 /var/log/ 下的纯文本文件中（如 syslog、auth.log）。在 3.2 节中，我们使用 logger 写入的日志，被 systemd-journald 捕获并存在 journal 中，同时它也会把该日志转发给 rsyslog，rsyslog 再将其写入 /var/log/syslog 文件中，所以同一条日志可以在两处被查看到。


2. SSH 提示 `Failed password` 能证明什么，不能证明什么？请结合本次记录回答。

   > 填写：SSH 提示 Failed password 能证明：尝试登录的账号（如 wrong）在主机上不存在，或者提供的密码不正确，导致 SSH 认证失败被拒绝。它不能证明：不能证明实际操作者就是该账号的真实拥有者（因为可能是恶意尝试），也不能证明系统存在漏洞或已被入侵。结合本次记录，只能说明有人（或某个程序）在 127.0.0.1 发起了一次密码错误的 SSH 登录尝试，但未成功。
