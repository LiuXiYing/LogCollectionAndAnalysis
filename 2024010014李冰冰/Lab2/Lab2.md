...
> 记录：journald 的实际运行状态为 __正在运行____。
...
> 记录：rsyslog 的实际运行状态为 ___正在运行___。
...
> 记录：Ubuntu 的日期和时间为 ____2026-09-11 14:54:55 CST__；时区为 ___Asia/Shanghai (CST, +0800)___；时间同步状态为 ____System clock synchronized: yes（已同步），NTP service: active__。
...
> 记录：`/var/log/auth.log` 是否存在？___存在___；实际输出：____auth.log exists__。
...
> 记录：本次 SSH 连接使用的 Ubuntu 虚拟机 IP 为 _192.168.225.128_____。
...
> 记录：22 端口是否正在监听？___是___；输出中的本地地址和端口为 ___**0.0.0.0:22、[::]:22**___。
...
> 记录：SSH 登录后的主机名为 __user123-VMware-Virtual-Platform____。
...
> 记录：当前目录为 __**/home/user123**____；是否确认已通过 Git Bash 登录 Ubuntu？____是__。
...
> 记录：`/var/log/journal/` 的检查结果为 ___目录存在___；`/run/log/journal/` 的检查结果为 ____目录存在__。
...
| :--- | :--- | :--- | :--- |
| /var/log/syslog | 系统全局综合日志，记录大部分系统服务运行信息 | 文本 | tail、cat、grep |
| /var/log/auth.log | 认证、登录、sudo 操作等安全审计日志 | 文本 | tail、cat、grep |
| /var/log/wtmp | 用户登录、登出历史记录 | 二进制 | last |
| /var/log/btmp | 登录失败记录 | 二进制 | lastb |
| /var/log/journal | systemd 的持久化日志存放目录 | 目录 | journalctl |
**先读取文本日志**
...
> 记录：选取的一条日志来自哪个程序或服务？___systemd-networkd_；记录了什么事件？___**网卡 ens33 成功获取载波与 IPv6 地址，网络枚举完成**___。
...
> 记录：实际查询起点为 __2026-09-11 08:00:00____；终点为 _2026-09-11 15:00:00___；观察到的事件或无记录情况为 _**输出多条内核、系统服务定时任务日志，包含 anacron 定时任务、fwupd 服务启动失败、内核性能相关提示**__。
...
> 记录：是否查到匹配日志？___是___；其中一条的内容或无记录提示为 ____**kernel: workqueue: psi_avgs_work hogged CPU for >10000us 4 times, consider switching to WQ_UNBOUND**__。
...
> 填写：`/var/log/wtmp`与`/var/log/btmp`是二进制格式日志，`cat`只能解析纯文本文件，直接读取会出现乱码。读取成功登录记录用`last`命令；读取失败登录记录使用`sudo lastb`命令。
