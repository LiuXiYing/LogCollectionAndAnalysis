...
> 填写：`user.notice` 属于 user 设施，不属于 auth/authpriv 认证设施。auth.log 只接收 auth、authpriv 设施的日志，user 设施日志匹配默认规则，写入 syslog，因此不会进入 auth.log。

> **自检题 ②**：sshd 的登录日志为什么**只**出现在 `auth.log`，不出现在 `syslog`？提示：看 `*.*;auth,authpriv.none` 这一行里的 `none`。

> 填写：配置规则 `*.*;auth,authpriv.none` 含义：接收所有设施日志，**但是排除 auth、authpriv 设施**。sshd 登录日志使用 auth/authpriv 设施，被这条规则过滤，不会写入 syslog；auth.log 单独匹配 auth/authpriv 设施，所以日志只保存到 auth.log。
...
| case=4 `local6.err` |/var/log/loglab/local6.log |
...
| :--- | :---: | :---: | :--- |
| case=1 `user.info` |是 | 否|`*.*`规则匹配，无 none 排除；user 设施不属于 auth/authpriv，不会写入 auth.log |
| case=2 `auth.info` |否 | 是|`auth,authpriv.*`匹配写入 auth.log；syslog 规则`*.*;auth,authpriv.none`排除 auth，不会进 syslog |
| case=3 `cron.info` |是 | 否| `*.*`规则匹配，无 none 排除；cron 不属于 auth/authpriv，不会写入 auth.log|
| case=4 `local6.err` | 是|否 |`*.*`规则匹配，无 none 排除；local6 不属于 auth/authpriv，不会写入 auth.log，此时还未配置 local6 单独日志文件 |
...
>
> 答：**能查到**。配置了`local6.* /var/log/local6.log`，local6 设施的日志会匹配这条规则写入 local6.log。
>
> ② `syslog` 里能查到吗？参照上面的流向图，说明同一条消息为什么三处都有记录。
>
> 答：**能查到**。rsyslog 是**多目标分发**，消息复制多份：匹配 local6 自定义规则写入 local6.log；同时`*.*`通用规则匹配，写入 syslog；同时 systemd-journal 捕获所有日志存入 journal，所以同一条日志在三处留存。
>
> ③ `journalctl` 里能查到吗？
>
> 答：**能查到**。journald 独立捕获系统全部日志，不受 rsyslog 配置影响，可以用 journalctl 检索该消息。
>
> ④ 新文件的属主、属组和权限分别是什么？
>
> 答：属主：`syslog`，属组：`adm`，权限：`-rw-r-----`（0640）
...
> 填写：不重启 rsyslog 时，日志继续写入 local6.log.1
...
>
> 答：状态词是 **(deleted)**。
含义：文件在文件系统上已经被重命名 / 删除，但**rsyslog 进程仍然持有这个文件的文件描述符**，内核不会真正释放这个 inode 占用的磁盘空间，进程依旧可以继续向这个文件写入数据。
...
>
> 答：`ocal6.log`、`local6.log.1`、`local6.log.2.gz`
...
1. facility 和 priority 分别表示什么？`mail.err` 匹配的是"只有 err"还是"err 及以上"？
   ...
   > 填写：facility 是**设施 / 设备类型**，标识日志来源模块；priority 是**优先级**，代表日志严重等级。`mail.err`匹配**err 及以上**更高级别的日志。

2. 为什么日志需要轮转，而不是一直往同一个文件里写？请至少说出两个理由。

   > 填写：日志持续写入单个文件会不断占用磁盘空间，容易耗尽存储；大日志文件查询、备份、清理都很慢。日志轮转可以拆分文件、限制大小，方便归档与删除。

3. `logrotate -d` 和 `logrotate -f` 的区别是什么？改完 logrotate 配置后，为什么标准动作是先 `-d`，再等定时器？

   > 填写：`logrotate -d`是**调试模式**，只模拟轮转，不修改真实文件；`logrotate -f`是**强制立即执行轮转**。修改配置后先用`-d`预校验，避免配置语法错误破坏日志，再等待定时任务自动执行。
