...
| 项目 | 你的填写内容 |
| :--- | :--- |
| 虚拟机 IP 地址 |192.168.30.128 |
| 网络模式 | NAT / 其他：NAT |
| ping `223.5.5.5` 是否成功 |成功 |
| ping `mirrors.tuna.tsinghua.edu.cn` 是否成功 |成功|
| 使用的软件源镜像站 |清华大学镜像站 mirrors.tuna.tsinghua.edu.cn |
...
| 项目 | 你的填写内容 |
| :--- | :--- |
| 宿主机内存 / CPU 核心 / 存放盘剩余空间 | 16.0GB；AMD Ryzen7 8845H（8 核 16 逻辑处理器）；剩余 84.5GB|
| 选择的配置档位 | 课程推荐档 |
| 虚拟 CPU 核心数 |2 |
| 虚拟内存 | 5.7Gi|
| 虚磁盘容量 |40G |
...
| CPU、内存、存储 | 至少 2 核、4GB、40GB，且与宿主机档位匹配 | 符合|
| VMware Tools、SSH、rsyslog | 软件包已安装，服务和功能检查通过 | 全部通过|
