...
> 填写：匹配选择器规则 `*.*;auth,authpriv.none`。这条规则匹配所有设施所有级别的日志，但排除`auth`、`authpriv`设施。logger 默认日志设施是`user`，不属于 auth/authpriv；`auth,authpriv.*`规则只接收 auth、authpriv 设施日志，所以不会写入 auth.log，最终写入`/var/log/syslog`。

> **自检题 ②**：sshd 的登录日志为什么**只**出现在 `auth.log`，不出现在 `syslog`？提示：看 `*.*;auth,authpriv.none` 这一行里的 `none`。

> 填写：sshd 登录日志属于`auth/authpriv`设施，匹配`auth,authpriv.*`规则，写入`/var/log/auth.log`；
规则`*.*;auth,authpriv.none`含义：接收所有设施日志写入 syslog，**但是丢弃 auth、authpriv 设施的日志**，因此 ssh 登录日志不会写入 syslog。
...
| :--- | :---: | :---: | :--- |
| case=1 `user.info` | 是|否 |user 设施没有独立日志规则，匹配全局`*.*`写入 syslog；不属于 auth 认证类设施，不会写入 auth.log |
| case=2 `auth.info` | 否|是 |auth 设施匹配`auth,authpriv.*`规则写入 auth.log；syslog 配置中用`none`排除 auth 类日志，因此不会出现在 syslog |
| case=3 `cron.info` | 是| 否|cron 设施没有单独配置日志文件，匹配全局`*.*`写入 syslog；不属于 auth 相关设施，不会写入 auth.log |
| case=4 `local6.err` | 是| 否| local6 是自定义设施，无专属日志匹配规则，由全局`*.*`捕获写入 syslog；不属于 auth 设施，不会写入 auth.log|
...
>
> 答：**可以查到**。执行`cat /var/log/loglab/local6.log`，能看到日志消息 `facility=local6 priority=info msg=hello`。
>
> ② `syslog` 里能查到吗？参照上面的流向图，说明同一条消息为什么三处都有记录。
>
> 答：**可以查到**。
原因：`logger`发送的日志消息，先提交给 systemd 的 journald；同时 rsyslog 会接收 syslog 消息。
rsyslog 有两套规则：
1. 自定义规则：将`local6.info`写入独立文件`/var/log/loglab/local6.log`；
2. rsyslog 系统默认配置，会把这条日志继续写入系统总日志`/var/log/syslog`；
journald（journalctl 读取）是独立日志系统，所有 syslog 消息都会被 journald 捕获保存，所以同一条消息会在**local6.log、syslog、journalctl**三处留存。
>
> ③ `journalctl` 里能查到吗？
>
> 答：**可以查到**。使用`sudo journalctl -t loglab -n 2`，可以看到 tag 为 loglab 的这条测试日志。
>
> ④ 新文件的属主、属组和权限分别是什么？
>
> 答：属主：`syslog`；属组：`adm`；权限：`-rw-r-----`（对应数字权限 640）。
...
>
> 答：有，输出显示正在处理该日志文件：rotating pattern: /var/log/loglab/local6.log after 1 days (7 rotations)
>
...
> ② 括号里的状态词是什么？它意味着什么？
>
> 答：状态词是 **deleted**；含义：文件在文件系统目录中已经被重命名移除，但是 rsyslog 进程仍然持有这个文件的句柄，文件不会被立刻释放删除，进程依旧向这个 inode 写入日志。
...
1. facility 和 priority 分别表示什么？`mail.err` 匹配的是"只有 err"还是"err 及以上"？
   ...
   > 填写：facility（设施）代表日志来源模块（如 mail、auth）；priority（优先级）代表日志严重级别。`mail.err`匹配**err 及以上**严重等级的日志。

2. 为什么日志需要轮转，而不是一直往同一个文件里写？请至少说出两个理由。

   > 填写：①防止日志文件持续膨胀占满磁盘空间；②大日志文件查询、压缩、归档速度很慢，轮转后拆分小文件便于管理与旧日志清理。

3. `logrotate -d` 和 `logrotate -f` 的区别是什么？改完 logrotate 配置后，为什么标准动作是先 `-d`，再等定时器？

   > 填写：`logrotate -d`是调试模式，仅模拟轮转、不实际操作；`-f`强制立刻执行轮转。修改配置先用`-d`检查语法错误，避免错误配置直接破坏日志，之后等待定时任务正常执行。
