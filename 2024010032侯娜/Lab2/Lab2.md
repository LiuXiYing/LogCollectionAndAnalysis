...
> 记录：journald 的实际运行状态为 _**active**_。
...
> 记录：rsyslog 的实际运行状态为 _**active**_。
...
> 记录：Ubuntu 的日期和时间为 **Thu 2026‑09‑10 08:23:08 CS**；时区为 __**Thu 2026‑09‑10 08:23:08 CST**__；时间同步状态为 _**yes**_。
...
> 记录：`/var/log/syslog` 是否存在？__**存在**__；实际输出：_**syslog exists**_。
...
> 记录：`/var/log/auth.log` 是否存在？__**存在**__；实际输出：__**auth.log exists**__。
...
> 记录：本次 SSH 连接使用的 Ubuntu 虚拟机 IP 为 __**192.168.250.128**__。
...
> 记录：22 端口是否正在监听？_**是**_；输出中的本地地址和端口为 __**0.0.0.0:22、[::]:22**__。
...
> 记录：SSH 登录后的主机名为 _**VMware‑Virtual‑Platform**_。
...
> 记录：当前目录为 _**/home/hn**_；是否确认已通过 Git Bash 登录 Ubuntu？_**是**_。
...
> 记录：`/var/log/journal/` 的检查结果为 ___**存在**___；`/run/log/journal/` 的检查结果为 ___**存在**___。
...
| :--- | :--- | :--- | :--- |
|/var/log/syslog|系统通用运行日志|文本|cat / tail|
|/var/log/auth.log|认证、登录相关日志|文本 |cat / tail |
|/var/log/wtmp |全部登录记录 |二进制 | last|
|/var/log/btmp|失败登录记录|失败登录记录 |lastb |
...
> 记录：选取的一条日志来自哪个程序或服务？_**gnome‑shell**_；记录了什么事件？__**窗口管理器修改按键绑定，覆盖原有按键配置**__。
...
> 记录：实际查询起点为 ___**2026‑09‑10 09:10:00**___；终点为 ___**2026‑09‑10 09:20:10**___；观察到的事件或无记录情况为 **sysstat系统统计任务启停、网卡ens33链路状态切换（断开、激活）**。
...
> 记录：是否查到匹配日志？_**是**_；其中一条的内容或无记录提示为 ___**kernel: CPUID marked event: 'cpu cycles' unavailable**___。
...
> 填写：
**/var/log/wtmp 、 /var/log/btmp 是二进制文件，直接用cat读取只会出现乱码。读取wtmp使用 last 命令，读取btmp使用 lastb 命令。**
