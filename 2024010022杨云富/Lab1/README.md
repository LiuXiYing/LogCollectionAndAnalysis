...
是否为教师指定版本	是
https://imgs/lab1-vmware-version.png
...
https://imgs/lab1-ubuntu-version.png
...
第六步：填写检查结果
项目	你的填写内容
虚拟机 IP 地址	192.168.75.128
网络模式	NAT
ping 223.5.5.5 是否成功	是
ping mirrors.tuna.tsinghua.edu.cn 是否成功	是
使用的软件源镜像站	清华源 (mirrors.tuna.tsinghua.edu.cn) 及 Ubuntu 官方安全源
sudo apt update 是否成功	是
联网是否合格	合格
https://imgs/lab1-network.png
...
第五步：填写检查结果
项目	你的填写内容
宿主机内存 / CPU 核心 / 存放盘剩余空间	16GB / 未提供 / 未提供
选择的配置档位	课程推荐档
虚拟 CPU 核心数	2
虚拟内存	3.8GiB
虚磁盘容量	40G
根分区可用空间	27G
资源分配是否符合对应档位	符合（2核、4GB、40GB）
https://imgs/lab1-resources.png
...
桌面分辨率是否能自动调整	是
SSH 是否 enabled / active	是 / 是（注：需手动启动 ssh 服务后）
22 端口是否监听	是
rsyslog 是否 enabled / active	是 / 是
/var/log/syslog 是否出现带学号姓名的测试日志	是
三项组件是否全部验收合格	合格
https://imgs/lab1-services.png
