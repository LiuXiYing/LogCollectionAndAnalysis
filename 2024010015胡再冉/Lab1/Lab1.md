...
是否为教师指定版本	是
VMware版本：VMware Workstation Pro 26H1 for Windows

执行操作：打开VMware，点击Help → About VMware Workstation，查看版本信息并截图。

![VMware版本截图](./imgs/lab1-vmware-version.png)
...
表格
项目	你的填写内容
Ubuntu 当前完整版本	Ubuntu 24.04 LTS
...
表格
项目	你的填写内容
宿主机内存 / CPU 核心 / 存放盘剩余空间	宿主机 16GB 内存，8 核 CPU，存放磁盘剩余空间充足
选择的配置档位	最低可用档
虚拟 CPU 核心数	2
虚拟内存	5.7Gi
虚磁盘容量	40G
...
ss -tulnp | grep 22
logger "lab1-check 学号:2024010015 姓名:胡再冉"
grep "lab1-check" /var/log/syslog
...
bash
运行
logger -t lab1-check "Lab1 rsyslog test 2024010015 胡再冉"
bash
运行
...
表格
项目	你的填写内容
VMware Tools 版本	open-vm-tools 对应版本
...
本次搭建虚拟机过程中，前期遇到磁盘扩容后系统损坏问题，重新新建 Ubuntu 虚拟机完成环境部署。安装完成后更换清华软件源，依次安装 open-vm-tools、openssh-server、rsyslog 组件。所有服务均正常启动，网络连通正常，rsyslog 日志写入功能验证成功。当前实验环境全部验收通过，可以开展后续日志分析相关实验。
截图要求
截图须清晰，菜单和终端文字可读。
终端截图应同时显示完整命令和其输出。
