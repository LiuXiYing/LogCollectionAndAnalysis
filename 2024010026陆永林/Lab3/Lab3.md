...
> 记录：本次使用的 Ubuntu 用户名为 ___studen26___。
...
> 记录：本次 SSH 连接的 Ubuntu 目标 IP 为 ___192.168.84.128___。
...
> 记录：本机主机名为 ___studen26-VMware-Virtual-Platform___（即 4W1R 中 Where 的取值）。
...
| :--- | :--- | :--- | :--- | :--- | :--- |
| 成功认证 |9 月 22 日 21:09:17 |studen26 | 192.168.84.1|Accepted password| /var/log/auth.log|
| 失败认证 |9 月 22 日 21:08:56 |studen26 |192.168.84.1 |Failed password |/var/log/auth.log |
...
| 两处记录有哪些共同字段或正文 | `student_id=2024010026 name=陆永林 action=write_test result=success`正文完全一致|
| 两处输出的主要区别 |journal 输出附带进程 PID、日志优先级等元数据；syslog 为传统 syslog 文本格式，元数据更少 |
...
```text
获取命令：sudo grep -F "student_id=2024010026" /var/log/syslog | tail -n 5
日志原文：2026-09-22T21:11:59.573454+08:00 studen26-VMware-Virtual-Platform lab3_read: student_id=2024010026 name=陆永林 action=write_test result=success
```

| 4W1R | 根据本人原始日志填写 |
| :--- | :--- |
| When 什么时候 |2026 年 9 月 22 日 21:11:59 |
| Where 在哪里 | 主机 `studen26-VMware-Virtual-Platform`|
| Who 谁 |标签 lab3_read，进程 PID3293；正文标识学号 2024010026，姓名陆永林 |
| What 做了什么 | 使用 logger 工具向系统日志写入一条测试消息|
| Result 结果如何 |消息写入成功，journald 与 rsyslog 均保存该日志记录 |

**用一两句话解释这个事件：**

> 填写：2026 年 9 月 22 日 21:11:59，在 studen26-VMware-Virtual-Platform 主机上执行 logger 命令生成测试日志，携带学号姓名信息，该日志同时被 journald 和 rsyslog 捕获存储。
...
日志原文：2026-09-22T21:08:58.736287+08:00 studen26-VMware-Virtual-Platform sshd[2807]: Failed password for studen26 from 192.168.84.1 port 53743 ssh2
2026-09-22T21:09:17.780165+08:00 studen26-VMware-Virtual-Platform sshd[2807]: Accepted password for studen26 from 192.168.84.1 port 53743 ssh2
2026-09-22T21:09:59.705714+08:00 studen26-VMware-Virtual-Platform sudo: studen26 : TTY=pts/0 ; PWD=/home/studen26 ; USER=root ; COMMAND=/usr/bin/grep -E 'Accepted|Failed password' /var/log/auth.log
2026-09-22T21:59:48.931638+08:00 studen26-VMware-Virtual-Platform sudo: studen26 : TTY=pts/0 ; PWD=/home/studen26 ; USER=root ; COMMAND=/usr/bin/grep -E 'Accepted|Failed password' /var/log/auth.log
```

| 4W1R | 根据本人原始日志填写 |
| :--- | :--- |
| When 什么时候 |2026 年 9 月 22 日 21:08:56 |
| Where 在哪里 |主机`studen26-VMware-Virtual-Platform`|
| Who 谁 |sshd 服务进程 PID2807；登录账号 studen26，来源客户端 IP：192.168.84.1 |
| What 做了什么 | 来自 192.168.84.1 的客户端尝试 SSH 密码登录 studen26 账号|
| Result 结果如何 |`Failed password`，本次密码认证失败 |

**用一两句话解释这个事件：**

> 填写：2026 年 9 月 22 日 21:08:56，IP 为 192.168.84.1 的客户端尝试 SSH 登录 studen26 用户，输入密码错误，认证失败。仅凭日志不能确定操作人真实身份。
...
日志原文：2026‑09‑22 21:14:15 status installed htop:amd64 3.3.0‑4build1
```

| 4W1R | 根据本人原始日志填写 |
| :--- | :--- |
| When 什么时候 |2026 年 9 月 22 日 21:14:15 |
| Where 在哪里 |Ubuntu 虚拟机，日志文件`/var/log/dpkg.log` |
| Who 谁 | dpkg 软件包管理程序，日志未记录执行命令的用户账号|
| What 做了什么 | 记录 htop 软件包安装过程，标记软件包状态变更|
| Result 结果如何 |状态`installed`，htop 3.3.0-4build1 安装完成 |

**用一两句话解释这个事件：**

> 填写：2026 年 9 月 22 日 21:14:15，dpkg 工具记录 htop 软件包安装完成，日志仅记录包状态，未记录是谁执行了安装命令。
...
1. systemd-journald 与 rsyslog 各负责什么？结合 3.2 节的结果，解释它们为什么可以同时保留同一条日志。
   ...
   > 填写：systemd-journald 是 systemd 内置日志服务，接收系统所有日志消息，以二进制格式保存，使用 journalctl 读取；rsyslog 接收 journald 转发的日志，将日志持久化为文本文件存放在`/var/log`目录。同一条日志先发送给 journald，journald 一方面保存二进制日志，另一方面转发消息给 rsyslog 写入文本日志，因此两处都保存了完全相同的日志内容。

2. SSH 提示 `Failed password` 能证明什么，不能证明什么？请结合本次记录回答。

   > 填写：可以证明：来自 192.168.84.1 的主机向 studen26 账号发起 SSH 登录尝试，输入的密码和正确密码不匹配，本次认证失败。不能证明：无法确认操作人的真实身份；无法区分是本人输错密码还是外部暴力破解；也不能保证来源 IP 没有被伪造。
