...
>
> 答：20-ufw.conf 、 21-cloudinit.conf  、 60-loglab-local6.conf
...
> 填写：这条日志的 facility 是`user`，不是`auth`。rsyslog 配置里 auth.log 只接收 auth/authpriv 设施的日志，user.notice 匹配 syslog 的通用规则，所以写入`/var/log/syslog`。

> **自检题 ②**：sshd 的登录日志为什么**只**出现在 `auth.log`，不出现在 `syslog`？提示：看 `*.*;auth,authpriv.none` 这一行里的 `none`。

> 填写：syslog 的规则`*.*;auth,authpriv.none`表示接收所有日志，**但是排除 auth、authpriv 设施的日志**。sshd 登录日志属于 authpriv，因此被这条过滤，只写入 auth.log，不再进 syslog。
...
| 命令 | 先猜：会落到哪个文件？ |
| :--- | :--- |
| case=1 `user.info` |syslog |
| case=2 `auth.info` |auth.log |
| case=3 `cron.info` |syslog |
| case=4 `local6.err` | syslog|
...
| :--- | :---: | :---: | :--- |
| case=1 `user.info` | 是|否 |`user` 设施命中 `*.*` 规则 → 写入 syslog；`user` 不等于 `auth`，不被 `auth,authpriv.none` 排除，所以进 syslog；但 `user` 也不等于 `auth,authpriv`，不命中 auth.log 的规则，所以不进 auth.log。|
| case=2 `auth.info` |否 |是 |`auth` 设施命中 `auth,authpriv.*` 规则 → 写入 auth.log；而 syslog 那条规则带 `auth,authpriv.none`，明确排除了 auth 设施，所以 [auth.info](https://auth.info) 被挡在 syslog 之外。|
| case=3 `cron.info` |是 | 否|`cron` 设施命中 `*.*` 规则 → 写入 syslog；`cron` 不等于 `auth`，不被 `auth,authpriv.none` 排除，所以进 syslog；但 `cron` 不等于 `auth,authpriv`，不命中 auth.log 规则，所以不进 auth.log。 |
| case=4 `local6.err` |是 |否 |`local6` 设施命中 `*.*` 规则 → 写入 syslog；`local6` 不等于 `auth`，不被 `auth,authpriv.none` 排除，所以进 syslog；但 `local6` 不等于 `auth,authpriv`，不命中 auth.log 规则，所以不进 auth.log。 |
...
>
> 答：能查到。已经配置 rsyslog 将[local6.info](https://local6.info)日志写入`/var/log/loglab/local6.log`，`cat`命令也成功读出消息。
>
> ② `syslog` 里能查到吗？参照上面的流向图，说明同一条消息为什么三处都有记录。
>
> 答：能查到。rsyslog 收到这条日志后，**会匹配所有满足条件的规则，不是匹配一条就停止**，因此同时写入 local6.log 和 syslog。另外 logger 发送的消息会被 systemd 的 journald 独立捕获保存，于是 journalctl 也能看到，三处都存在记录。
>
> ③ `journalctl` 里能查到吗？
>
> 答：能查到。journald 独立接收系统日志，logger 发出的消息会同时送入 journald，所以 journalctl 可以检索到。
>
> ④ 新文件的属主、属组和权限分别是什么？
>
> 答：属主：`syslog`，属组：`adm`，权限：`-rw-r-----`。
...
>
> 答：inode A(名字叫local16.log)
>
> ② 括号里的状态词是什么？它意味着什么？
>
> 答：括号里的状态词是deleted，它意味着旧文件被压缩，原文件被删除。
...
>
> 答：2026-09-24T09:23:10.438047+08:00 hzd-VMware-Virtual-Platform loglab: after-rotate test-2
>
> ② 此时 `/var/log/loglab/` 的文件清单是？
>
> 答：总计 12
1579577-rw-r-----1 syslog adm 86 9月 24 09:25 local6.log1579580-rw-r----- 1 syslog adm 1059月 24 09:23 local6.log.1.gz1579579-rw-r-----1 syslog adm 123 9月 24 09:13 local6.log.2.gz
...
1. facility 和 priority 分别表示什么？`mail.err` 匹配的是"只有 err"还是"err 及以上"？
   ...
   > 填写：facility（设施）代表日志消息来自哪一类子系统；priority（优先级）代表消息严重级别。`mail.err`匹配**err 及更高级别**的日志。

2. 为什么日志需要轮转，而不是一直往同一个文件里写？请至少说出两个理由。

   > 填写：防止单个日志文件无限增大占满磁盘空间。同时，分割后的旧日志可压缩、定时删除，方便归档与检索。

3. `logrotate -d` 和 `logrotate -f` 的区别是什么？改完 logrotate 配置后，为什么标准动作是先 `-d`，再等定时器？

   > 填写：`logrotate -d`是调试模式，**只模拟，不实际执行轮转**；`-f`是强制立刻轮转，不管是否满足轮转条件。修改配置先用`-d`检查语法错误，避免配置写错造成故障，之后等待定时任务自动执行正式轮转。
