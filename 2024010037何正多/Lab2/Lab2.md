...
> 记录：Ubuntu 的日期和时间为 __四 2026-09-10 13:34:50 UTC
____；时区为 __Asia/Shanghai (CST, +0800)
____；时间同步状态为 ___active___。
...
> 记录：`/var/log/syslog` 是否存在？__是____；实际输出：___syslog exists
___。
...
> 记录：`/var/log/auth.log` 是否存在？___是___；实际输出：__auth.log exists
____。
...
> 记录：本次 SSH 连接使用的 Ubuntu 虚拟机 IP 为 __192.168.222.128 
____。
...
> 记录：22 端口是否正在监听？___是___；输出中的本地地址和端口为 ___0.0.0.0:22   4096___。
...
> 记录：SSH 登录后的主机名为 __hzd-VMware-Virtual-Platform
____。
...
> 记录：当前目录为 _/home/hzd
___；是否确认已通过 Git Bash 登录 Ubuntu？__是___。
...
> 记录：`/var/log/journal/` 的检查结果为 ____drwxr-sr-x+ 2 root systemd-journal   40  9月 10 21:31 /run/log/journal
__；`/run/log/journal/` 的检查结果为 __drwxr-sr-x+ 3 root systemd-journal 4096  8月 27 10:01 /var/log/journal
____。
...
| :--- | :--- | :--- | :--- |
|/var/log/journal/ |systemd 日志	|目录 | journalctl|
|/var/log/auth.log| sudo 提权、su、SSH 登录等认证行为|文本	| cat / tail -f / grep|
|/var/log/syslog |系统综合运行日志，记录服务启停（如 sysstat-collect） |	文本 |tail / less / grep |
|/var/log/kern.log | 内核日志， CPUID marked event unavailable、piix4_smbus ... SMBus Host Controller not enabled 等内核警告|文本	 | cat / grep|
...
> 记录：实际查询起点为 __2026-09-09 08:00:00"____；终点为 ___ "2026-09-09 12:00:00" ___；观察到的事件或无记录情况为 __ --no-pager
-- No entries --
____。
...
> 记录：是否查到匹配日志？__是___；其中一条的内容或无记录提示为 __ file ../../../gio/gfileinfo.c: line 1765 (g_file_info_get_icon): should not be reached
____。
...
> 填写：/var/log/wtmp 和 /var/log/btmp 均为二进制文件，用 cat 查看会输出乱码，需用专用命令解析：
last​ —— 读取 /var/log/wtmp，查看成功登录记录
lastb​ —— 读取 /var/log/btmp，查看失败登录记录（需 root 权限）
