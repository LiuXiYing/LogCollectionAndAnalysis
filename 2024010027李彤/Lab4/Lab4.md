...
> 填写：消息设施为user，不匹配auth,authpriv.*这条写入 auth.log 的规则；匹配*.*;auth,authpriv.none，会写入/var/log/syslog。

> **自检题 ②**：sshd 的登录日志为什么**只**出现在 `auth.log`，不出现在 `syslog`？提示：看 `*.*;auth,authpriv.none` 这一行里的 `none`。

> 填写：sshd 日志使用authpriv设施，命中auth,authpriv.*写入 auth.log；*.*;auth,authpriv.none规则把 auth、authpriv 排除，不会写入 syslog。
...
| :--- | :---: | :---: | :--- |
| case=1 `user.info` |是 |否 | rsyslog 配置中*.*匹配该日志，无 none 排除；user 设施没有单独专属日志，写入 syslog；不属于 auth 类，不会写入 auth.log|
| case=2 `auth.info` |否 |是 |匹配auth,authpriv.*规则，写入 auth.log；syslog 的规则设置了auth,authpriv.none，把 auth 类日志排除，不会进入 syslog|
| case=3 `cron.info` |是 |否 |*.*匹配该日志，无 none 排除；cron 日志会写入 syslog；不属于 auth 设施，不会写入 auth.log |
| case=4 `local6.err` |是 |否 |*.*匹配该日志，当前配置没有给 local6 设置单独日志文件，写入 syslog；不属于 auth 设施，不会写入 auth.log |
...
>
> 答：能查到，包含 msg=hello 日志
>
> ② `syslog` 里能查到吗？参照上面的流向图，说明同一条消息为什么三处都有记录。
>
> 答：能查到。消息会被 systemd-journald 捕获存入 journal；rsyslog 有两条匹配规则，local6.* 写入 local6.log，通用*.*规则写入 syslog，一条日志匹配多条规则，因此三处都保存记录
>
> ③ `journalctl` 里能查到吗？
>
> 答：能查到
...
> ② 括号里的状态词是什么？它意味着什么？
>
> 答：状态词是 deleted。含义：文件在目录中已经被重命名（从 local6.log 改名为 local6.log.1），目录项删除；但是 rsyslog 进程仍然持有这个文件的打开句柄，内核不会真正释放磁盘空间，进程依旧可以继续向这个旧文件写入日志。
...
1. facility 和 priority 分别表示什么？`mail.err` 匹配的是"只有 err"还是"err 及以上"？
   ...
   > 填写：facility（设施）代表日志来源模块，priority（优先级）代表日志严重等级。mail.err匹配err 及更高级别的日志。

2. 为什么日志需要轮转，而不是一直往同一个文件里写？请至少说出两个理由。

   > 填写：日志持续写入会让单个日志文件体积无限增大，占用大量磁盘空间。大文件查看、检索、备份都会变慢，轮转可以拆分旧日志并支持压缩归档。

3. `logrotate -d` 和 `logrotate -f` 的区别是什么？改完 logrotate 配置后，为什么标准动作是先 `-d`，再等定时器？

   > 填写：-d是调试模式，仅模拟轮转、不实际修改文件；-f强制立刻执行轮转。修改配置先用-d可以提前检查语法错误，避免配置写错直接破坏日志，之后等待系统定时自动轮转。
