...
> 记录：journald 的实际运行状态为 _active____。
...
> 记录：Ubuntu 的日期和时间为 2026-09-10 08:39:57______；时区为_Asia/Shanghai (CST, +0800)_____；时间同步状态为 yes______。
...
> 记录：`/var/log/syslog` 是否存在？存在______；实际输出：syslog exists______。
...
> 记录：`/var/log/auth.log` 是否存在？存在_____；实际输出：_auth.log exists____。
...
> 记录：本次 SSH 连接使用的 Ubuntu 虚拟机 IP 为 192.168.222.128 ______。
...
> 记录：SSH 服务的实际运行状态为 active______。
...
> 记录：22 端口是否正在监听？是_____；输出中的本地地址和端口为 _0.0.0.0:22____。
...
> 记录：SSH 登录后的主机名为 litong-VMware-Virtual-Platform_____。
...
> 记录：当前目录为 /home/litong______；是否确认已通过 Git Bash 登录 Ubuntu？_是____。
...
> 记录：`/var/log/journal/` 的检查结果为 __存在____；`/run/log/journal/` 的检查结果为 存在_____。
...
| :--- | :--- | :--- | :--- |
|/var/log/syslog |系统全局综合运行日志，记录系统服务事件|文本 |tail / less / grep |
|/var/log/auth.log |认证安全日志，登录、sudo 权限操作记录 |文本 |tail / less / grep |
|/var/log/wtmp |用户登录、注销、系统开关机历史记录 |二进制文件 |last |
|/var/log/btmp |登录失败记录（当前为空文件） |二进制文件 |	sudo lastb |
|/var/log/lastlog | 所有用户最近一次登录信息|二进制文件 |lastlog |
...
> 记录：选取的一条日志来自哪个程序或服务？CRON______；记录了什么事件？_定时任务服务打开 root 用户会话，执行计划任务____。
...
> 记录：实际查询起点为 2026-09-10 08:00:00_____；终点为 2026-09-10 12:00:00_____；观察到的事件或无记录情况为 __存在系统服务启动日志、定时任务 cron 日志，sssd-sudo.socket 出现依赖启动失败的警告。 __。
...
> 记录：是否查到匹配日志？是_____；其中一条的内容或无记录提示为 9月 10 08:27:39 litong-VMware-Virtual-Platform kernel: core: CPUID marked event: 'cpu cycles' unavailable_____。
...
> 填写：/var/log/wtmp 和 /var/log/btmp 是二进制日志文件，不是普通文本，cat 读取会乱码。查看 wtmp 用last，查看 btmp 用lastb
