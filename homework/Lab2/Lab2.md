...
> 记录：本次 SSH 连接使用的 Ubuntu 虚拟机 IP 为 ______。
...
> 记录：22 端口是否正在监听？______；输出中的本地地址和端口为 ______。
...
> 记录：当前目录为 ______；是否确认已通过 Git Bash 登录 Ubuntu？______。
...
> 记录：`/var/log/journal/` 的检查结果为 ______；`/run/log/journal/` 的检查结果为 ______。
...
> 记录：实际查询起点为 ______；终点为 ______；观察到的事件或无记录情况为 ______。
...
> 记录：是否查到匹配日志？______；其中一条的内容或无记录提示为 ______。
