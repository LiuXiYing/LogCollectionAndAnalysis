...
> 记录：Ubuntu 的日期和时间为2026-09-10 08:48:28 CST；时区为Asia/Shanghai (CST, +0800)；时间同步状态为yes。
...
> 记录：`/var/log/auth.log` 是否存在？是；实际输出：cho "auth.log exists"auth.log exists。
...
> 记录：本次 SSH 连接使用的 Ubuntu 虚拟机 IP 为 192.168.252.128。
...
> 记录：22 端口是否正在监听？是；输出中的本地地址和端口为 0.0.0.0:22 和[::]:22。
...
> 记录：SSH 登录后的主机名为 yanglian-VMware-Virtual-Platform。
...
> 记录：当前目录为 /home/yanglian；是否确认已通过 Git Bash 登录 Ubuntu？是。
...
> 记录：`/var/log/journal/` 的检查结果为 存在；`/run/log/journal/` 的检查结果为 存在。
...
| :--- | :--- | :--- | :--- |
|/var/log/syslog|系统全局综合日志，记录内核、系统服务、应用的运行信息、警告与错误，是 Linux 核心系统日志|文本|cat、less、tail|
|/var/log/auth.log|系统认证安全日志，记录用户登录登出、sudo 权限校验、SSH 连接等身份验证事件|文本|cat、grep、tail|
|/var/log/boot.log|系统启动日志，记录开机过程中各服务的启动状态、加载结果与报错，用于排查启动故障|文本|cat、less、head|
|/var/log/dmesg|内核消息落盘文件，存储内核启动、硬件检测、驱动加载的内核输出信息|文本|cat、less，也可直接用dmesg命令|
|/var/log/journal|systemd 统一管理的结构化日志目录，存储全量系统、服务、内核日志，支持持久化与高效检索|目录|journalctl|
...
> 记录：选取的一条日志来自哪个程序或服务？9月 10 09:15:07 yanglian-VMware-Virtual-Platform systemd[1]: NetworkManager-dispatcher.service: Deactivated successfully.；记录了什么事件？NetworkManager-dispatcher.service 网络调度服务被成功停用。。
...
> 记录：实际查询起点为 2026-09-10 08：00：00；终点为2026-09-1012:00:00；观察到的事件或无记录情况为该时间段内系统运行正常，记录到多类事件：NetworkManager-dispatcher 网络调度服务完成启动与停用的全生命周期；多轮 CRON 定时任务（系统统计、每小时例行任务）依次执行，对应 root 会话正常开闭；sysstat-collect 系统账户统计服务启动并执行完成后正常停用；用户 yanglian 通过 sudo 执行 journalctl 日志查询操作，权限会话流程完整，全程无异常报错。。
...
> 记录：是否查到匹配日志？是；其中一条的内容或无记录提示为 dbus-daemon[1107]: Unknown group "power" in message bus configuration file本次通过journalctl -p warning -b筛选本次启动的警告及以上级别日志，成功匹配到多条对应记录，除此以外还包含内核 workqueue 任务 CPU 占用超时警告、gnome-shell 图标接口断言警告等同级别警告日志。
...
> 填写： /var/log/wtmp  和  /var/log/btmp  是二进制格式的登录记录数据库文件，而非纯文本文件，直接用  cat  读取只会输出乱码。其中 wtmp 记录所有成功登录/注销事件，应使用  last  命令读取；btmp 记录所有失败登录尝试，应使用  lastb  命令读取。
