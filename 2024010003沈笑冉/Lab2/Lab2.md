...
> 记录：22 端口是否正在监听？___是___；输出中的本地地址和端口为 __0.0.0.0：22__[::]:22__。
...
> 记录：SSH 登录后的主机名为 __wsnnnn-VMware-Virtual-Platform____。
...
> 记录：当前目录为 _/home/wsnnnn_____；是否确认已通过 Git Bash 登录 Ubuntu？_是_____。
...
> 记录：`/var/log/journal/` 的检查结果为 ___存在___；`/run/log/journal/` 的检查结果为 ___存在___。
...
> 记录：选取的一条日志来自哪个程序或服务？__sudo____；记录了什么事件？__用户 wsnnnn 在 pts/0 终端，使用 sudo 提升至 root 权限，执行 journalctl -n 30 --no-pager 命令查询系统日志____。
