...
> 填写：这条日志设施是 user ，不是 auth/authpriv 。
 auth,authpriv.*  规则只捕获auth、authpriv设施的日志写入auth.log；
而  *.*;auth,authpriv.none  表示所有设施日志，但是排除auth、authpriv，写入syslog。
user.notice匹配这条规则，所以日志进入syslog，不会进入auth.log。

> **自检题 ②**：sshd 的登录日志为什么**只**出现在 `auth.log`，不出现在 `syslog`？提示：看 `*.*;auth,authpriv.none` 这一行里的 `none`。

> 填写：sshd登录日志属于 auth/authpriv 设施。
...
| :--- | :---: | :---: | :--- |
| case=1 `user.info` |是 | 否| user设施，匹配 *.*;auth,authpriv.none 写入syslog，不属于auth类，不会写入auth.log|
| case=2 `auth.info` | 否|是 |匹配 auth,authpriv.* 写入auth.log；syslog规则里 auth,authpriv.none 排除auth日志，所以不会进syslog |
| case=3 `cron.info` |是 | 否|cron设施，本机无独立cron.log，匹配 *.*;auth,authpriv.none 写入syslog；不属于auth，不会进auth.log |
| case=4 `local6.err` |是 |否 |local6没有单独配置分流规则，匹配 *.*;auth,authpriv.none 写入syslog，不属于auth |
...
>
> 答：可以查到，执行cat命令能够读取到logger发送的日志内容。
>
> ② `syslog` 里能查到吗？参照上面的流向图，说明同一条消息为什么三处都有记录。
>
> 答：可以查到。
logger产生日志，先交给 systemd-journal 存入journal日志；日志同时送入rsyslog服务。rsyslog会匹配所有满足条件的配置规则：匹配自定义 local6.* 规则写入 local6.log ，同时匹配系统默认规则写入 /var/log/syslog 。journald是独立日志收集系统，单独捕获日志，不受rsyslog配置影响，因此一条日志会在这三个位置保存。
>
> ③ `journalctl` 里能查到吗？
>
> 答：可以查到。journald独立收集系统所有日志，与rsyslog相互独立，因此可以检索到本次日志。
>
> ④ 新文件的属主、属组和权限分别是什么？
>
> 答：文件权限 -rw-r----- ，属主 syslog ，属组 adm 。
...
>
> 答：识别到了，输出可见  rotating pattern: /var/log/loglab/local6.log 
>
> ② 有没有报 `parent directory has insecure permissions`？
>
> 答：没有该警告，配置正常，不需要额外加 su syslog adm 
...
> 填写：轮转完成之后，rsyslog会把下一条local6日志写进 旧文件（local6.log.1）
...
>
> 答：轮转后 local6.log 的inode编号：1049956
...
> ② 括号里的状态词是什么？它意味着什么？
>
> 答：状态词是 deleted 。代表文件在目录中已经被重命名删除，但是rsyslog进程仍然持有这个文件的句柄，还能继续往这个inode写入日志，目录上已经看不到这个文件名，属于孤儿文件。
...
1. facility 和 priority 分别表示什么？`mail.err` 匹配的是"只有 err"还是"err 及以上"？
   ...
   > 填写：答：facility（设施/设备）代表日志的来源模块；priority（优先级）代表日志严重等级。 mail.err 匹配err及以上更严重的级别（err、crit、alert、emerg），不只是err。


2. 为什么日志需要轮转，而不是一直往同一个文件里写？请至少说出两个理由。

   > 填写：答：①防止单个日志文件无限增大，耗尽磁盘空间；②旧日志自动归档压缩，便于历史日志管理、查看与清理，避免日志文件过大读写缓慢。


3. `logrotate -d` 和 `logrotate -f` 的区别是什么？改完 logrotate 配置后，为什么标准动作是先 `-d`，再等定时器？

   > 填写：答： -d 是debug调试模式，只模拟、不实际修改文件，用来检查配置语法； -f 是force强制模式，立刻执行真实轮转，无视轮转周期。修改配置先用 -d 校验配置有无语法错误，避免配置错误导致日志损坏，确认无误后再等待系统定时器自动执行。
