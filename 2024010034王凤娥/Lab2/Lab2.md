...
> 记录：journald 的实际运行状态为 <u>active</u>。
...
> 记录：rsyslog 的实际运行状态为  <u>active</u>。
...
> 记录：Ubuntu 的日期和时间为  <u>2026-09-10 08:30:17 CST</u>；时区为  <u>Asia/Shanghai (CST, +0800)</u>；时间同步状态为 <u>yes</u>。
...
> 记录：`/var/log/syslog` 是否存在？<u>是</u>；实际输出：<u>syslog exists</u>。
...
> 记录：`/var/log/auth.log` 是否存在？<u>是</u>；实际输出：<u>auth.log exists</u>。
...
> 记录：本次 SSH 连接使用的 Ubuntu 虚拟机 IP 为 <u>192.168.79.128</u>。
...
> 记录：22 端口是否正在监听？<u>是</u>；输出中的本地地址和端口为 <u>0.0.0.0:22、[::]:22</u>
...
> 记录：SSH 登录后的主机名为 <u>wfe-VMware-Virtual-Platform</u>
。
...
> 记录：当前目录为 <u>home/wfe</u>；是否确认已通过 Git Bash 登录 Ubuntu？<u>是</u>。
...
> 记录：`/var/log/journal/` 的检查结果为 <u>drwxr-sr-x+ 3 root systemd-journal 4096  8月 30 22:24 /var/log/journal</u>；`/run/log/journal/` 的检查结果为<u>drwxr-sr-x+ 2 root systemd-journal   40  9月 10 08:14 /run/log/journal</u>
。
...
| :--- | :--- | :--- | :--- |
| /var/log/syslog| 系统全局运行日志|文本（ASCII text） | cat / tail|
|/var/log/auth.log | 登录、SSH 身份认证日志| 文本（ASCII text）| cat / tail|
|/var/log/wtmp | 所有用户登录历史记录| 二进制（data）| last|
|/var/log/btmp | 失败登录记录| 二进制（empty/data）| lastb|
| /var/log/lastlog| 每个用户最近一次登录信息| 二进制（data）| lastlog|
...
> 记录：选取的一条日志来自哪个程序或服务?<u>pam_unix（sudo认证模块</u>；记录了什么事件？<u>用户wfe执行sudo，打开root会话</u>。
...
> 记录：实际查询起点为 <u>2026-09-10 08:00:00</u>；终点为 <u>2026-09-10 09:00:00</u>;观察到的事件或无记录情况为 <u>存在多条系统日志，包含sudo会话打开/关闭、内核启动、图形界面相关日志</u>。
...
> 记录：是否查到匹配日志？<u>是</u>；其中一条的内容或无记录提示为 <u>gnome-shell[2578]: Can't update stage views actor unnamed </u>。
...
> 填写：/var/log/wtmp 和 /var/log/btmp 是二进制文件，不是纯文本，cat直接读取只会输出乱码。查看 /var/log/wtmp 使用 last 命令，查看 /var/log/btmp 使用 lastb 命令。
