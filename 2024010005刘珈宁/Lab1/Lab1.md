...
| 项目 | 你的填写内容 |
| :--- | :--- |
| 虚拟机 IP 地址 |192.168.165.128|
| 网络模式 |NAT|
| ping `223.5.5.5` 是否成功 |成功|
| ping `mirrors.tuna.tsinghua.edu.cn` 是否成功 |成功|
| 使用的软件源镜像站 |mirrors.tuna.tsinghua.edu.cn（清华大学开源软件镜像站）|
...
> 填写：
> 遇到的问题：openssh‑server 软件包存在，但 ssh.service 服务文件缺失，无法设置开机自启。
> 解决方法：执行`sudo apt reinstall openssh‑server‑y`重装 openssh‑server，之后设置 ssh 开机自启；安装 open‑vm‑tools‑desktop 实现虚拟机窗口自适应与复制粘贴功能。当前虚拟机全部验收项合格，环境正常，可以继续后续实验。
