...
> 填写：配置规则 auth,authpriv.* /var/log/auth.log 只匹配设施为 auth、authpriv 的日志；这条日志是 user.notice，设施是 user，不匹配 auth 的规则。会被 *.*;auth,authpriv.none 这条全局规则捕获，写入 /var/log/syslog。

> **自检题 ②**：sshd 的登录日志为什么**只**出现在 `auth.log`，不出现在 `syslog`？提示：看 `*.*;auth,authpriv.none` 这一行里的 `none`。

> 填写：sshd 登录日志属于 auth 设施，优先匹配 auth,authpriv.* /var/log/auth.log，写入 auth.log；而 syslog 的规则是 *.*;auth,authpriv.none，auth,authpriv.none 表示丢弃 auth、authpriv 类型日志，所以不会再写入 syslog。
...
| :--- | :---: | :---: | :--- |
| case=1 `user.info` | 是 | 否 | 匹配*.*;auth,authpriv.none，代表所有设施所有级别，但排除 auth、authpriv。user 不属于 auth/authpriv，写入 syslog，没有规则写入 auth.log |
| case=2 `auth.info` | 否 | 是 | auth,authpriv.*逗号表示 “或”，匹配 auth 设施全部级别，写入 auth.log；*.*;auth,authpriv.none分号排除 auth，不会写入 syslog |
| case=3 `cron.info` | 是 | 否 | 匹配*.*;auth,authpriv.none，cron 不属于 auth/authpriv，写入 syslog；无规则将 cron 日志写入 auth.log |
| case=4 `local6.err` | 是 | 否 | 匹配*.*;auth,authpriv.none，local6 不属于 auth/authpriv，写入 syslog；当前没有local6.*选择器，不会写入 auth.log |
...
>
> 答：能查到，文件中包含 msg=hello 的测试日志。
>
> ② `syslog` 里能查到吗？参照上面的流向图，说明同一条消息为什么三处都有记录。
>
> 答：syslog 里可以查到。logger 发送日志写入本机 syslog 套接字，消息会被两个模块同时接收：
>1.systemd-journald 捕获，存入 journal 二进制日志（journalctl 查看）；
>2.rsyslogd 接收日志，依次匹配配置规则：
>(1)匹配60-loglab-local6.conf的local6.*，写入自定义文件/var/log/loglab/local6.log；
>(2)匹配50-default.conf默认全局规则*.*，写入/var/log/syslog。
>所以同一条日志会在三个位置同时保存。
>
> ③ `journalctl` 里能查到吗？
>
> 答：能查到，可以检索到 tag 为 loglab、内容含 msg=hello 的日志。
>
> ④ 新文件的属主、属组和权限分别是什么？
>
> 答：权限一般是 -rw-r--r--；属主：syslog；属组：adm。
...
> ② 括号里的状态词是什么？它意味着什么？
>
> 答：状态词：deleted。含义：文件在文件系统目录里已经被删除 / 改名压缩，目录上看不见了；但是 rsyslog 进程仍然持有该文件的文件句柄，内核还保留着这个文件实体，进程依然可以继续向这个 inode 写入数据。
...
1. facility 和 priority 分别表示什么？`mail.err` 匹配的是"只有 err"还是"err 及以上"？
   ...
   > 填写：facility 是设施 / 模块，标记日志来源（如 mail、local6）；priority 是优先级，代表日志严重等级。mail.err匹配err 及以上更高级别的日志。

2. 为什么日志需要轮转，而不是一直往同一个文件里写？请至少说出两个理由。

   > 填写：防止日志文件无限增大耗尽磁盘空间；便于归档、压缩、清理旧日志，同时避免超大日志文件打开、查看缓慢。

3. `logrotate -d` 和 `logrotate -f` 的区别是什么？改完 logrotate 配置后，为什么标准动作是先 `-d`，再等定时器？

   > 填写：-d是调试模式，只模拟轮转、不实际修改文件；-f强制立刻执行真实日志轮转。先-d校验配置无语法错误，避免错误配置被定时任务自动执行引发故障。
