...
| 项目 | 你的填写内容 |
| :--- | :--- |
| 宿主机内存 / CPU 核心 / 存放盘剩余空间 | 32GB内存/16核/D盘剩余456GB可用|
...
| 虚拟机联网 | 具有 IP 和默认路由，IP 联通与 DNS 解析正常 |是 |
| 国内软件源 | 已换成国内镜像站，`sudo apt update` 成功 |是 |
| CPU、内存、存储 | 至少 2 核、4GB、40GB，且与宿主机档位匹配 |是 |
| VMware Tools、SSH、rsyslog | 软件包已安装，服务和功能检查通过 | 是|

简要说明你遇到的问题、解决方法，以及当前环境是否可以继续完成后续实验：

> 填写：部署时 apt 官方源访问较慢，更换国内镜像源后sudo apt update执行成功。虚拟机网络、SSH、VMware Tools、rsyslog 均配置正常，硬件参数满足要求。各项验收通过，环境就绪，可以开展后续实验
