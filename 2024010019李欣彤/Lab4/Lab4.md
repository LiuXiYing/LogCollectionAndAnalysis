...
>
> 答：`/etc/rsyslog.d/` 下除了 `50-default.conf`，还有 `20-ufw.conf`。
...
> 填写：`logger` 默认是 `user.notice`，匹配选择器规则 `*.*;auth,authpriv.none`。该规则表示所有设施所有级别日志，排除 auth、authpriv 设施日志，写入 `/var/log/syslog`；这条日志属于 user 设施，不属于 auth/authpriv，不会匹配 `auth,authpriv.*` 规则，因此写入 syslog，不会进入 auth.log。

> **自检题 ②**：sshd 的登录日志为什么**只**出现在 `auth.log`，不出现在 `syslog`？提示：看 `*.*;auth,authpriv.none` 这一行里的 `none`。

> 填写：sshd 登录日志属于 auth/authpriv 设施，匹配 `auth,authpriv.*` 规则写入 `/var/log/auth.log`。而 `*.*;auth,authpriv.none` 规则里的 `auth,authpriv.none` 代表**不接收 auth 和 authpriv 的任何级别日志**，所以这类日志不会被写入 syslog。
...
| case=3 `cron.info` |/var/log/syslog |
...
| :--- | :---: | :---: | :--- |
| case=1 `user.info` |是 |否 |不属于 auth/authpriv 设施，匹配`*.*;auth,authpriv.none`写入 syslog；不匹配`auth,authpriv.*`，不会写入 auth.log |
| case=2 `auth.info` | 否| 是| 匹配`auth,authpriv.*`写入 auth.log；`*.*;auth,authpriv.none`排除 auth 设施日志，因此不会出现在 syslog|
| case=3 `cron.info` | 是| 否|cron 没有单独生效日志规则，不属于 auth/authpriv，匹配`*.*;auth,authpriv.none`写入 syslog，不匹配 auth 相关规则 |
| case=4 `local6.err` |是 |否 |local6 无单独日志配置，不属于 auth/authpriv，匹配`*.*;auth,authpriv.none`写入 syslog，不匹配 auth 相关规则 |
...
>
> 答：能，可以看到 `facility=local6 priority=info msg=hello` 这条日志
>
> ② `syslog` 里能查到吗？参照上面的流向图，说明同一条消息为什么三处都有记录。
>
> 答：**syslog 里能查到。**
logger 发送的日志送入系统套接字，`systemd-journald`捕获一份存入 journal；rsyslog 收到消息，**同时匹配两条独立规则**：`60-loglab-local6.conf`的`local6.*`写入`local6.log`；`50-default.conf`的`*.*;auth,authpriv.none`匹配 local6，写入 syslog。一条日志可匹配多条 rsyslog 规则，多条规则都会执行，因此三处都保存记录。
>
> ③ `journalctl` 里能查到吗？
>
> 答：能，查询到标签 loglab 对应的 msg=hello 日志。
>
> ④ 新文件的属主、属组和权限分别是什么？
>
> 答：权限`-rw-r-----`，属主`syslog`，属组`adm`。
...
> ② 这台虚拟机下次自动轮转的时间是？
>
> 答：Fri 2026-09-25（2026 年 09 月 25 日）
...
>
> 答：有，识别成功。输出 `Handling 1 logs`，并读取到轮转规则 `rotating pattern: /var/log/loglab/local6.log`
>
> ② 有没有报 `parent directory has insecure permissions`？
>
> 答：**没有出现该报错**，目录权限安全，不需要添加 `su syslog adm`。
...
> 填写：logrotate 做轮转时，会把旧的 `local6.log` 重命名为 `local6.log.1.gz`，然后新建一个空白 `local6.log`。
...
>
> 答：状态词是 **deleted**。
含义：文件已经被重命名 / 轮转，在文件系统目录上看不到原来的名字了；**但是 rsyslog 进程仍然持有这个旧文件的文件描述符（句柄），内核不会真正删除这个文件，进程还能继续往里面写数据**。所以新发送的日志会继续写入这个已经被轮转改名的旧文件，而不是新生成的 `local6.log`。
...
1. facility 和 priority 分别表示什么？`mail.err` 匹配的是"只有 err"还是"err 及以上"？
   ...
   > 填写：facility 是日志的设施（来源分类），priority 是日志的严重级别。`mail.err`匹配**err 级别以及比 err 更严重的级别**。

2. 为什么日志需要轮转，而不是一直往同一个文件里写？请至少说出两个理由。

   > 填写：持续写入单个日志文件会不断占用磁盘；文件过大时查看、检索日志困难，轮转可以控制文件大小并自动清理过期日志。

3. `logrotate -d` 和 `logrotate -f` 的区别是什么？改完 logrotate 配置后，为什么标准动作是先 `-d`，再等定时器？

   > 填写：`-d`是调试演练，仅打印计划操作，不改动文件；`-f`强制立刻执行真实轮转。先使用`-d`用来校验配置语法，避免错误配置被定时任务触发引发故障。
