...
> 记录：Ubuntu 的日期和时间为 ___2026-09-10 08:36:17___；时区为 __Asia/Shanghai___；时间同步状态为 __yes___。
...
> 记录：`/var/log/syslog` 是否存在？___存在___；实际输出：__syslog exists____。
...
> 记录：本次 SSH 连接使用的 Ubuntu 虚拟机 IP 为 __192.168.250.128____。
...
> 记录：22 端口是否正在监听？__是___；输出中的本地地址和端口为 __地址为：0.0.0.0：22，端口为：[::]:22__。
...
> 记录：SSH 登录后的主机名为 __lxt-VMware-Virtual-Platform__。
...
> 记录：当前目录为 ___/home/lxt___；是否确认已通过 Git Bash 登录 Ubuntu？__是____。
...
| :--- | :--- | :--- | :--- |
/var/log/syslog记录系统通用运行日志文本tail / cat/var/log/auth.log用户登录、身份认证相关日志文本tail / cat/var/log/wtmp记录历史登录记录二进制last/var/log/btmp记录登录失败信息二进制lastb/var/log/journalsystemd 持久化日志存放目录目录journalctl
...
> 记录：选取的一条日志来自哪个程序或服务？__kernel__；记录了什么事件？
事件描述
__`core: CPUID marked event: 'cpu cycles' unavailable`，内核提示虚拟机 CPU 的 cpu cycles 性能事件不可用，属于 VMware 虚拟机常见警告。__。
...
> 记录：实际查询起点为 __2026-09-10 12:00___；终点为 ___2026-09-10 12:30___；观察到的事件或无记录情况为 ___时间段内主要是网卡 ens33 反复断开、重新连接，网络状态切换；系统定时任务执行，DNS 服务切换传输协议，NTP 时间同步请求超时，系统进入睡眠后唤醒，还有 AppArmor 拦截固件更新服务读取文件的拒绝审计记录___。
...
> 记录：是否查到匹配日志？___是___；其中一条的内容或无记录提示为 __kernel: core: CPUID marked event: 'cpu cycles' unavailable__。
...
> 填写：`/var/log/wtmp` 和 `/var/log/btmp` 是**二进制日志文件**，不是纯文本，cat 直接读取会显示乱码。读取 wtmp 用`last`，读取 btmp 用`lastb`。
